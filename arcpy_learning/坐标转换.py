...
if __name__ == '__main__':
    with open(r'C:\Users\Administrator\Desktop\wenhua_goeinfo.csv', 'r', encoding='utf-8') as file:
        data = csv.reader(file)
        for i in data:
            if i[2] != 'none':
                x, y = getWgs84xy(float(i[2]), float(i[3]))
            else:
                x, y = ['none', 'none']
            data = pd.DataFrame(data=[[i[0], i[1], x, y]],
                                columns=['地名', '地址', '经度', '纬度'])
